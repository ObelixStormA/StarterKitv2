...
export default function Edit({ user, roles }: { user: User; roles: string[] }) {
    const { t } = useLocale();
    ...
    return (
        <AuthenticatedLayout header={<h2 className="heading-2 text-secondary-900">{t('users.edit.title')}</h2>}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-semibold text-secondary-900 mb-2">
                                {t('users.field.new_password')}{' '}
                                <span className="text-secondary-400 font-normal">({t('common.optional')})</span>
                            </label>
                            <input
                                type="password"
                                value={data.password}
